...
exports.setVisitClaim = async (req, res) => {
    ...
    try {
        ...
        const visitCheck = await db.collection('visitSubmit').find(dQuery).toArray();
        ...
        await db.collection('visitearns').updateOne(vQuery, { $set: { count: totalVisit } })
...
exports.createDirectLink = async (req, res) => {
    ...
    try {
        ...
        const totalVisit = parseInt(visit?.count) + parseInt(1)
        const directCheck = await db.collection('directSubmit').find(dQuery).toArray();
        if (directCheck.length > 0) {
            return res.send({ Error: `Already completed this task` })
        }
        const result = await db.collection('directSubmit').insertOne({ uid: id, jobid: vId, visitTime: formattedDhakaTime, subTime: '', prof: '', status: 'Pending', amount: visit.amount })
        await db.collection('directlinks').updateOne(vQuery, { $set: { count: totalVisit } })
        res.send(result);
    } catch (err) {
        res.status(500).json({ error: 'Internal Server Error' });
    }
...
exports.updateDirectLink = async (req, res) => {
    ...
    try {
        const result = await db.collection('directSubmit').updateOne(vQuery, { $set: { prof, subTime:formattedDhakaTime, status: 'Submit' } })
        res.send(result);
    } catch (err) {
        res.status(500).json({ error: 'Internal Server Error' });
    }
